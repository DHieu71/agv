...
class Message:
    ...
    def addData(self, xyJetbot, xyProduct):
        self.xyJetbot = xyJetbot
        self.xyProduct = xyProduct
    ...
    def __arrivedPark(self):
        point=  Point(self.xyJetbot)
        if self.typeOfProdcut == "kho hàng 1" and self.location["area1"].contains(point):
            self.data['targetPosition'] = 'storage1'
            self.check = True
        elif self.typeOfProdcut == "kho hàng 2" and self.location["area2"].contains(point):
            self.data['targetPosition'] = 'storage2'
            self.check = True
        elif self.typeOfProdcut == "kho hàng 3" and self.location["area3"].contains(point):
